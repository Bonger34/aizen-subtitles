...
2. **判定源文件的存放处** —— 下面这 10 个 JSON 是那 195 条修正、255 条时间戳重排与 84 条重复清理的可复现依据，**不要删**。
...
> 原先还有一份 `带外垃圾条目清单.md`（26 条"画面不是台词、库里却存了文本"的待办）。
...
**前六个一旦删掉，校验链的收尾两步会直接报错** —— `q_audit.py` 读不到输入，
`q_reframe.py` 找不到待重抽清单。用 `scripts/pipeline/q_audit.py` 自检：

```
修正总数 195 | 已落盘 170 | 已回退 2 | 已删条目 23 | 异常 0
```

> ⚠️ `q_retime_applied.json` 与 `q_dedup_applied.json` 都是**必须**的：修正记录
> （`q_apply_result` / `q_manual_verdicts`）里存的时间戳是**改动前**的，而被删掉的 23 条
> 本身带着修正记录。少了这两张表，`q_audit.py` 会按老时间戳查不到、或找不到已删条目，
> 把它们长期误报成「异常」，掩盖真正的漏落盘。
>
...
`.gitignore` 在这里只放行 `*.md` 和上表那 10 个 JSON，其余全是本地产物：
