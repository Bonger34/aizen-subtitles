...
> （`q_apply_result` / `q_manual_verdicts`）里存的时间戳是**改动前**的，而被删掉的条目
> 本身带着修正记录。少了这些表，`q_audit.py` 会按老时间戳查不到、或找不到已删条目，
> 把它们长期误报成「异常」，掩盖真正的漏落盘。
>
> 这类清理还会继续做，所以 `q_audit.py` 用的是通配 `review/q_dedup*_applied.json`：
> **每轮加一张表即可，不用再改脚本**。
>
