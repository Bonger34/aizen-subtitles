...
通道1 全帧原图 / 通道2 字幕带原图 / 通道3 字幕带反色二值化
匹配：分行 LCS>=0.5 或包含（引号清理），取最佳命中帧。
用法: python extract_frames_scan7.py
"""
import json
import os
import re
...
def main():
    ...
    print('PP-OCRv5/v6 server · GPU 就绪', flush=True)
    ...
    for ep, (video, missing) in channels.items():
        ...
        for r in missing:
            ...
            while t_ms <= end_ms:
                ...
                _, bb = cv2.threshold(255 - gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                variants = [frame, crop, cv2.cvtColor(bb, cv2.COLOR_GRAY2BGR)]
                for v in variants:
                    res = ocr(v)
                    txt = ''.join(res.txts) if res.txts else ''
                    if txt and match_lines(lib_lines, txt):
                        ocr_c = QUOTE_RE.sub('', txt).replace(' ', '')
    ...
    print(f'\n结果: v5/v6引擎命中 {len(fixed)} | 仍未命中 {len(still)}', flush=True)
    with open(os.path.join(BASE, 'review', 'frames_scan7_fix.json'), 'w', encoding='utf-8') as fh:
        json.dump({'fixed': fixed,
                   'still': [{'ep': e, 'ts': r.get('timestamp'), 'text': r.get('text', '')[:30]}
                             for e, r in still]},
                  fh, ensure_ascii=False, indent=1)
    print('记录: review/frames_scan7_fix.json')
