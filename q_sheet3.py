...
用法: python q_sheet3.py [每张行数=12] [序号列表, 如 1,4,5,7]
输出: review/q_sheet3_<k>.jpg
"""
import json
import os
import sys
...
def main():
    ...
    rows = json.load(open(os.path.join(REVIEW, 'q_pick3.json'), encoding='utf-8'))
    if len(sys.argv) > 2:
        keep = {int(x) for x in sys.argv[2].split(',')}
        rows = [r for i, r in enumerate(rows, 1) if i in keep]
    ...
    for k in range(0, len(rows), per):
        chunk = rows[k:k + per]
        cv2_img = Image.new('RGB', (1560, len(chunk) * ROW_H), (24, 24, 24))
        dr = ImageDraw.Draw(cv2_img)
        for i, r in enumerate(chunk):
            ...
            if img is not None:
                ...
                cv2_img.paste(Image.fromarray(cv2.cvtColor(band[:bh], cv2.COLOR_BGR2RGB)), (548, y + 2))
            dr.text((8, y + 4), f'{k + i + 1}. {r["ep"]} {r["ts"]} sim={r["sim"]:.2f} sup={r["sup"]}',
                    font=f1, fill=(255, 220, 120))
            dr.text((8, y + 40), r['desc'][:44], font=f2, fill=(160, 200, 255))
            dr.text((8, y + 76), '旧: ' + r['old'][:16], font=f1, fill=(230, 230, 230))
            if len(r['old']) > 16:
                dr.text((8, y + 108), '    ' + r['old'][16:32], font=f2, fill=(200, 200, 200))
            dr.text((8, y + 138), '新: ' + r['new'][:16], font=f1, fill=(150, 255, 150))
            if len(r['new']) > 16:
                dr.text((8, y + 170), '    ' + r['new'][16:32], font=f2, fill=(140, 230, 140))
            dr.line([(0, y + ROW_H - 1), (1560, y + ROW_H - 1)], fill=(90, 90, 90), width=1)
        out = os.path.join(REVIEW, f'q_sheet3_{k // per + 1}.jpg')
        cv2_img.save(out, quality=92)
        print('saved', out)
