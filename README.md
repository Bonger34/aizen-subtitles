...
|---|---:|---|
| 集数 | 25 | 全剧 |
| 归档台词 | **6693** 条 | `subtitle/`，站点唯一数据源 |
| 爱染诚登场条目 | **533** 条 | 说话时画面含爱染诚 |
| 帧图 | **6690** 张 | 960×540 |
| 帧图覆盖率 | **100.0%** | 6693 条全部有对应帧文件，无占位 |
| 站点体积 | **398 MB** | `docs/`，其中帧图 396.9 MB |

数据的可信度不是靠"看起来对"得来的。仓库里保留了完整的验证链，任何时候都能重跑：

```bash
python scripts/pipeline/rebuild_map.py         # 6693 -> 6693 键, 缺帧 0
...
node   scripts/pipeline/verify_search.js       # 399 命中 / 缺帧 0
python scripts/pipeline/q_audit.py             # 历史修正 195 处(落盘 167 · 回退 2 · 已删 26), 异常 0
```
...
│
├─ subtitle/                 【权威库】25 集 / 6693 条台词，站点的唯一数据源
...
│   ├─ aizen_frames.js       533 张"含爱染诚"的帧名单（编译期输入）
│   └─ frames/               6690 张帧图
│
